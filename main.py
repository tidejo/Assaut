...
class Player(pygame.sprite.Sprite):
    def __init__(self, image, x, y, angle):
        ...
        self.angle = angle
        self.bullet_speed = 6                             # Powerup possibility
        self.reload_time = 10 ** 8
...
class Asteroid(pygame.sprite.Sprite):
    ...
    def __init__(self, image, x, y):
        pygame.sprite.Sprite.__init__(self)
        self.default_image = scale_image(image, random.choice([0.4,0.5,0.6,0.7,0.8]))
        ...
        self.speed = 0.2
        self.alive = True
        self.target = random.getrandbits(1)
    ...
    def rotate_by_player_position(self):
        if self.target == 0:
            player_position = player1.rect.centerx, player1.rect.centery
        else:
            player_position = player2.rect.centerx, player2.rect.centery
...
class PlayerManager:
    ...
    def handle_keyboard_player_inputs(self):
        ...
        if keys_pressed[K_w]:
            self.player.accelerate('up')
        if keys_pressed[K_s]:
            self.player.accelerate('down')
        if keys_pressed[K_a]:
            self.player.accelerate('left')
        if keys_pressed[K_d]:
            self.player.accelerate('right')

    def handle_mouse_player_inputs(self):
        mouse_buttons_pressed = pygame.mouse.get_pressed()
        if mouse_buttons_pressed[0]:
            if time.time_ns() > (self.last_shot + self.player.reload_time):
                self.player.fire()
                self.last_shot = time.time_ns()
    ...
    def DEBUG_show_player_ray(self):
        pygame.draw.line(window_surface, (255, 0, 0), (self.player.rect.centerx, self.player.rect.centery), (pygame.mouse.get_pos()[0], pygame.mouse.get_pos()[1]))
...
player_image = scale_image(player_image, 0.7)

player1 = Player(player_image, WIDTH//3, HEIGHT//2, 0)
player2 = Player(player_image, 2*WIDTH//3, HEIGHT//2, 0)
player_group = pygame.sprite.Group(player1, player2)

bullet_image = pygame.image.load(os.getcwd() + '/sprites/objects/bullet.png')
bullet_image = scale_image(bullet_image, 2)
bullets_group = pygame.sprite.Group()

explosion_images = [
            scale_image(pygame.image.load(filename), 1)
            for filename in (
...
asteroid_image = scale_image(asteroid_image,  1.5)
num_asteroids = 5
asteroids = [
    Asteroid(
        asteroid_image,
...
player1_manager = PlayerManager(player1, game_settings)
player2_manager = PlayerManager(player2, game_settings)
    ...
    for event in pygame.event.get():
        ...
        if event.type == KEYDOWN:
            if event.key == K_ESCAPE:
                game_ended = True
    player1_manager.manage()
    player2_manager.manage()
